...
(function ($, document, window, undefined) {
  ...
  "use strict"; // Name the plugin so it's only in one place

  var pluginName = 'emotionsRating';
  var $element; // Default options for the plugin as a simple object
  ...
  function Plugin(element, options) {
    this.element = element; // Merge the options given by the user with the defaults

    this.settings = $.extend({}, defaults, options); // Attach data to the element
  ...
  $.extend(Plugin.prototype = {
    ...
    emotionStyle: function emotionStyle() {
      var styles = "." + this.styleCode + "{margin-right:3px;border-radius: 50%;cursor:pointer;opacity:0.3;display: inline-block;font-size:" + this.settings.emotionSize + "px; text-decoration:none;line-height:0.9;text-align: center;color:" + this.settings.color + "}";
    ...
    clearEmotion: function clearEmotion(content) {
    ...
    showEmotion: function showEmotion(count) {
    ...
    manageHover: function manageHover() {
      var self = this;
    ...
    initalRate: function initalRate(count) {
      var self = this;
      self.showEmotion(count);
    ...
    appendInput: function appendInput(count) {
      var total = '';
      ...
      var _input = "<input type='hidden' class='" + this.code + "' name='" + this.settings.inputName + "' value='" + total + "' />";

      var div = this.elementContainer;
      div.append(_input);
    },
    updateInput: function updateInput(count) {
      var _input = this.elementContainer.find("input." + this.code + "");

      _input.val(count);
    },
    transformImgsToSVG: function transformImgsToSVG() {
  ...
  var getEmotion = function getEmotion(_emotions, count) {
    var onlyIndex = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : false;
    var emotion;
