...
(function (a) {
  ...
  a.extend(b.prototype = {
    ...
    renderEmotion: function renderEmotion() {
      var a = this.settings.count,
          b = this.settings.useCustomEmotions,
          d = e[this.settings.bgEmotion];
      b && (d = "<img src='" + this.settings.bgEmotion + "' class='custom-" + this.styleCode + "'>");

      for (var f, g = "<div class='" + this.containerCode + "'>", h = 1; h <= a; h++) {
        f = "<div class='" + this.styleCode + "' data-index='" + h + "'>" + d + "</div>", g += f;
      }
    ...
    clearEmotion: function clearEmotion(b) {
      if (!this.settings.disabled) {
        var c = this.settings.useCustomEmotions,
            d = e[b];
    ...
    showEmotion: function showEmotion(a) {
      this.clearEmotion(this.settings.bgEmotion);
      var b = this.settings.useCustomEmotions,
          c = f(this.settings.emotions, a, b);
      b && (c = this.settings.emotions[c], c = "<img src='" + c + "' class='custom-" + this.styleCode + "'>");

      for (var d = 0; d < a; d++) {
        this.elementContainer.find("." + this.styleCode + "").eq(d).css("opacity", 1), this.elementContainer.find("." + this.styleCode + "").eq(d).html(c);
      }
